...
public class AjaxController {
	...
	*/
	...
	/*
	 * 2. 응답할 데이터를 문자열로 리턴
	 * 	  => HttpServletResponse를 안 쓸수 있음!
	 * 	  단, 문자열 리턴하면 원래는 포워딩 방식이였음 => 응답뷰로 인식해서 해당 뷰페이지를 찾을꺼임
	...
	public String ajaxMethod1(String name, int age) {
		String responseData = "응답 문자열 : " + name + "은(는)" + age + "살 입니다.";
		return responseData; // ResponseBody가 없으면 jsp로 던져주므로 responseData.jsp로 반환하게되어 404에러 발생
	}
	*/
	...
	public void ajaxMethod1(String name, int age, HttpServletResponse response) throws IOException {
		...
		// JSONArray => [값, 값, 값, 값, ..] => 자바에서의 ArrayList와 유사		// 인덱스 개념
		// JSONObject => {키:값, 키:값, 키:값, ..} => 자바에서의 HashMap과 유사	// 보따리 개념
		...
		jObj.put("age", age); // {age:20, name:"차은우"}						// 순서 무시하고 담김
	...
	*/
	...
	public String ajaxMethod2(int num) {
		...
		// Member m = mService.selectMember(num); 했다고 가정하고
	...
	*/
	...
	@ResponseBody
	@RequestMapping(value="ajax3.do", produces="appplication/json; charset=utf-8")
	public String ajaxMethod3() {
		
		// ArrayList<Member> list = mService.selectMember(); 조회했다고 가정하면 여러명의 정보가 리스트에 담김
		ArrayList<Member> list = new ArrayList<Member>();
		list.add(new Member("user01", "pass01", "차은우", 20, "01011112222"));
		list.add(new Member("user02", "pass02", "주지훈", 21, "01033332222"));
		list.add(new Member("user03", "pass03", "이재훈", 22, "01068781111"));
