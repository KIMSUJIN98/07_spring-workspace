...
public class MemberController {
	...
	@Autowired
	private BCryptPasswordEncoder bcryptPasswordEncoder;												// 필요에 따라 자동적으로 생성하고, 제거함
	...
	*/
	...
	public String updateMember(Member m, HttpSession session, Model model) {
		...
		if(result > 0) { // 수정 성공
			...
			// session에 loginUser 키값으로 덮어 씌워야함
			...
			// 마이페이지 url 재요청
			return "redirect:myPage.me";	// "redirect:/"를 쓰게되면 메인페이지로 이동함
	...
	public String deleteMember(String userPwd, String userId, HttpSession session, Model model) {
		...
		//System.out.println("평문 비번 (입력값) : " + userPwd);
		//System.out.println("암호문 비번 (DB 조회값) : " + encPwd);
		...
		if(bcryptPasswordEncoder.matches(userPwd, encPwd)) {
			...
			if(result > 0) { // 탈퇴처리 성공 => session loginUser 지움, alert 메시지 담아 메인페이지 url 재요청
				...
				session.removeAttribute("loginUser"); 		// session.invalidate(); 사용시 모든 세션이 날라감 => alertMsg도 날라가면서 메시지가 출력되지 않음.
		...
		}else { // 비번틀림 => 비번이 틀림을 알려주고 마이페이지 보여지게
			session.setAttribute("alertMsg", "비밀번호를 잘못입력하셨습니다. 확인해주세요!");
	...
	public String idCheck(String checkId) { // 스프링에서는 앞단의 변수명과 동일하게 지정하면, key값을 별도 셋팅하지 않아도 그대로 그 값을 가져와 사용할 수 있다.
		...
		*/
		...
		return count > 0 ? "NNNNN" : "NNNNY";	// ResponseBody를 써야만 jsp가 아닌 응답데이터로써 인식할 수 있다.
